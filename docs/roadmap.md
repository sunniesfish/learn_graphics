...
- 💡 셰이더 디버깅은 값을 색으로 출력하는 게 정석이다 — 궁금한 값을 색 채널 하나(예: 빨강)에 실어 화면에 찍고 밝기로 읽는다. print가 없다. (`gl_FragColor`는 WebGL1/GLSL ES 1.00 문법이다. WebGL2 정석인 `#version 300 es`에선 `out vec4` 변수를 직접 선언해 대입한다.)
